raise 'The stdio_socat scenario requires a Rake task named \'executable\'' unless Rake.application[:executable]
...
BlinkerVars.socat_port = port = random_number 49152..65535
...
generated_file "opt/#{name}/start.sh" do
  puts '#!/bin/sh'
  puts %(/usr/bin/socat tcp-listen:#{port},reuseaddr exec:"stdbuf -i0 -oL -eL /opt/#{name}/#{executable} #{BlinkerVars.executable_args||""}",stderr)
end
